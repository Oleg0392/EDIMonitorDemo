...
namespace EDIMonitorDemoData.Repositories
{
    public class KonturOrderRepository : IKonturOrderRepository
    {
        ...
        private string InboxPathTest { get; set; } = @"\\EDI-MGN\zakaz\EXPIMP\Loading\SKBKontur\Inbox\Новая папка\";
        ...
        public KonturOrder[] LoadInboxTest()
        {
            LoadKonturOrders(InboxPathTest);
            return Items;
        }
                ...
                if (files.Length > 0)
                {
                    ...
                    for (int i = 0; i < files.Length; i++)
                    {
                        try
                        {
                            ...
                            {
                                FileName = file.Name,
                                OrderNumber = document.SelectSingleNode("/eDIMessage/order").Attributes["number"].Value,
